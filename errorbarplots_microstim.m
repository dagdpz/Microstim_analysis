function errorbarplots_microstim(monkey,X_par,Y_par,Selection,reduce_to_residuals)
COLOR=jet(16);
...
for k=1:max(numel(x_val),numel(y_val))
    ...
    unique_x_values=unique(comparison.(x_val{k}));
    x_range=max(unique_x_values)-min(unique_x_values);
    errorbar_width=x_range/30;
    for n=1:numel(unique_x_values)
        current_x_value=unique_x_values(n);
        hold on
        current_y_values=comparison.(y_val{k})(comparison.(x_val{k})==current_x_value);
        mean_y=nanmean(current_y_values);
        sem_y=sterr(current_y_values);
        ebh(n)=errorbar(current_x_value,mean_y,sem_y,sem_y);
        
        
        set(ebh(n),'Marker','s','MarkerFaceColor','r','MarkerEdgeColor','r','Color','r','Linewidth',3,'MarkerSize',8);
        h_pre_children                                          = get(ebh(n),'children');
        Xdata                                                   = get(h_pre_children(2),'Xdata');
        tempo                                                   = 4:3:length(Xdata);
        tempo(3:3:end) = [];
        xleft                                                   = tempo; xright = tempo+1;
        Xdata(xleft)                                            = [current_x_value-errorbar_width current_x_value-errorbar_width];
        Xdata(xright)                                           = [current_x_value+errorbar_width current_x_value+errorbar_width];
    ...
    text(x_min+3*x_step, y_max-3.5*y_step,      ['Std of runs = ',  num2str(round(std(comparison.(y_val{k}))*100)/100)],'fontsize',fontsize-2,'FontName',fonttype);
