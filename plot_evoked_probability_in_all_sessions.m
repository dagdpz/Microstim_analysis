function plot_evoked_probability_in_all_sessions

monkeys={'Curius','Linus'};
Conditions={'_L_CH','_R_CH','_L_IN','_R_IN'};
windows={'_early','_late'};
N_previous_sessions=0;
bins=[0:0.1:1];
...
for monkey = 1:numel(monkeys);
    updated_xls=strcat(monkeys{monkey}, '_updated_parameters');    
    ...
    All_sessions            =mastertable_excel_num(:,idx.session);
    Unique_sessions=unique(All_sessions);
    for N_current_session=1:numel(Unique_sessions)
        current_session=Unique_sessions(N_current_session);
        session_idx=All_sessions==current_session;
        N_total_session=N_previous_sessions+N_current_session;
        for win = 1:numel(windows)
            window_N_stim=0;
            window_N_evoked=0;
            for con = 1:numel(Conditions)
                condition_N_stim_field=['N_stim' windows{win} Conditions{con}];
                condition_N_evoked_field=['N_evoked' windows{win} Conditions{con}];
                idx.(condition_N_stim_field)=DAG_find_column_index(mastertable_excel_raw,condition_N_stim_field);
                idx.(condition_N_evoked_field)=DAG_find_column_index(mastertable_excel_raw,condition_N_evoked_field);
                condition_N_stim=mastertable_excel_num(session_idx,idx.(condition_N_stim_field));
                condition_N_evoked=mastertable_excel_num(session_idx,idx.(condition_N_evoked_field));
                window_N_stim=window_N_stim + condition_N_stim;
                window_N_evoked=window_N_evoked + condition_N_evoked;
            end
            
        N_stim_per_session.(['in' windows{win}])(N_total_session)=sum(window_N_stim);        
        N_evoked_per_session.(['in' windows{win}])(N_total_session)=sum(window_N_evoked);
        end
    end
    N_previous_sessions=N_total_session;
end

evoked_saccades_figure_early=figure('units','normalized','outerposition',[0 0 1 1]);
he=hist(N_evoked_per_session.in_early./N_stim_per_session.in_early,bins);
bar(bins,he,1,'FaceColor', 'r', 'linewidth', linewidth)
%set(he, 'FaceColor', 'r', 'linewidth', linewidth);
%set(gca, 'xlim', [-0.01 0.1], 'FontSize',fontsize, 'linewidth', linewidth);
box on
ylabel({'Sessions'},'FontSize',fontsize,'FontName',fonttype)
xlabel({'Probability of evoking saccades'},'FontSize',fontsize,'FontName',fonttype)
title('p_e_v_o_k_e_d histogram for early stimulation windows')

export_fig(['Probability of evoking saccades histogram acorss all sessions, early'], '-pdf', '-transparent') % pdf by run

% evoked_saccades_figure_late=figure('units','normalized','outerposition',[0 0 1 1]);
% hist(N_evoked_per_session.in_late./N_stim_per_session.in_late,bins)
% ylabel({'Sessions '},'FontSize',fontsize,'FontName',fonttype)
% xlabel({'Probability of evoking saccades'},'FontSize',fontsize,'FontName',fonttype)
% title('p_e_v_o_k_e_d histogram for late stimulation windows')
% 
% a=1;
% 
% 
...
% 
% 
% if ismember(Y_par,{'Probability of evoking saccades','Probability of delaying saccades','Amplitudes of evoked saccades','Latencies of evoked saccades','Contraversive target selection','Contraversive target selection difference'})
%     Y_condition=[Y_condition selection];
% end
% if ismember(X_par,{'probability of evoking saccades','probability of delaying saccades','Amplitudes of evoked saccades'})
...
%     %end
%     
%     hp=get(hs(1),'children'); % handle for plot inside scaterplot axes
...
%         [RHO,PVAL] = corr(comparison.(x_val{k})(idx_comparison),comparison.(y_val{k})(idx_comparison));
%     else
%         [RHO,PVAL] = deal(NaN);
%     end
